...
3. `gateway` validates the Keycloak JWT.
4. For `n2-users` tokens, `gateway` resolves `tenant_name` to `tenant_id` through the `system` service.
5. `gateway` forwards trusted request context headers to internal services.
6. Tenant-scoped database queries use the forwarded `tenant_id`.
7. System status endpoints accept only `n2-system` tokens and do not use request-body tenant identifiers as a source of truth.
...
- Gateway validation is configured with per-realm issuer, audience, and JWKS URL settings.
...
- The current application expects `n2-users` tokens to include `tenant_name`, `roles`, and `authz_version`.
